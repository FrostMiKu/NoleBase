...
impl WorkspaceIndex {
        ...
        let mut tags = self
            ...
            .map(|entry| TagSummary {
                ...
                documents: entry
                    ...
                    .collect::<HashSet<_>>()
                    .len(),
                mentions: entry.occurrences.len(),
            })
            .collect::<Vec<_>>();
    ...
    fn search_tag(&self, normalized: &str) -> Vec<SearchHit> {
        let Some(entry) = self.tags.get(normalized) else {
            return Vec::new();
        };
        let mut occurrences = entry.occurrences.clone();
...
mod tests {
    ...
    fn requested_revision_discards_a_queued_stale_snapshot() {
        ...
        assert_eq!(latest.tags()[0].name, "fresh");
    }
}
