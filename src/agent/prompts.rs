...
pub(crate) fn skill_catalog_prompt(skills: &[Skill]) -> String {
    let mut prompt = String::from(
        "## Available skills\nSkills supplement the current request and do not grant tools or permissions. When the user names a skill or the request clearly matches its description, read `skill://<name>` with the `read` tool before following its workflow.\n",
    );
    if skills.is_empty() {
